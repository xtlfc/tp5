...
Page({
  ...
  async initOutgoingCall() {
    try {
      ...
      // 调用云函数创建通话
      const result = await cloudFunctionManager.callFunction('video-call', {
        action: 'create',
  ...
  async rejectCall() {
    try {
      await cloudFunctionManager.callFunction('video-call', {
        action: 'reject',
        callId: this.data.callId
      })
  ...
  // 切换视频
  toggleVideo() {
    const isMuted = !this.data.isLocalVideoMuted
    this.setData({ isLocalVideoMuted: isMuted })

    // 调用云函数更新状态
    cloudFunctionManager.callFunction('video-call', {
      action: 'updateVideo',
      callId: this.data.callId,
      muted: isMuted
    })

    this.showStatusTip(isMuted ? '已关闭摄像头' : '已开启摄像头')
  },
  ...
  onLocalVideoStateChange(e) {
    console.log('本地视频状态变化:', e.detail)
  },

  onRemoteVideoStateChange(e) {
    console.log('远程视频状态变化:', e.detail)
  },

  // 视频错误处理
  onLocalVideoError(e) {
    console.error('本地视频错误:', e.detail)
    this.showError('摄像头启动失败')
  },

  onRemoteVideoError(e) {
    console.error('远程视频错误:', e.detail)
    this.showStatusTip('对方视频连接异常')
  }
})