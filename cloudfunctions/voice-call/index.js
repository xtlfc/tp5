// cloudfunctions/voice-call/index.js
...
async function createCall(callId, callerId, calleeId) {
  try {
    // 生成音频流地址（这里需要根据实际的音频服务配置）
    const audioUrl = `rtmp://your-audio-server/live/${callId}`
    ...
    // 发送推送通知给被叫方
    await sendCallNotification(calleeId, callerId, 'voice')
